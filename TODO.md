...
### v1.6.35 - 代码回退与 i18n 优化保留
- [x] 回退失败的应用重命名修改（HydroCoder Desktop → CC Desktop）
- [x] 回退数据迁移逻辑（避免数据丢失风险）
- [x] 保留"智能体模式"中文翻译优化
- [x] 保留 Agent 模式欢迎界面改进
- [x] 保留 i18n 键冲突修复（main.developerWelcome 独立）
- [x] 数据目录保持在 `cc-desktop` 不变
- [x] 更新 CLAUDE.md 配置路径说明
