...
### 📝 开发说明
```bash
# 热重载开发模式（推荐）- Vite + Electron 并行启动
npm run dev

# 仅启动 Vite 开发服务器
npm run dev:vite

# 仅启动 Electron（需要先运行 dev:vite）
npm run dev:electron

# 生产构建
npm run build:vue   # 构建 Vue 页面
...
1. **查看此文件** (`TODO.md`) - 了解当前进度
2. **启动开发环境**: `npm run dev` (热重载，自动刷新)
3. **查看 CLAUDE.md** - 了解完整开发历史
