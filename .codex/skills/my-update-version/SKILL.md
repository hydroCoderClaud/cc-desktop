...
- 顺序必须固定为：先 `git commit` 成功，再 `git tag` 成功，再 `git push origin master` 成功，再 `git push origin vX.Y.Z` 成功，之后才允许推送其他 remote 或检查 CI。
...
- 推送顺序必须串行，且必须先完成 `origin/master`，再完成 `origin` 的单个版本 tag，最后才处理 `gitlab`、`local` 等其他 remote。
