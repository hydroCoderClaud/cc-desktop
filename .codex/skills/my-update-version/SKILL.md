...
description: 针对 cc-desktop 的版本发布 skill：检查 master 与工作区状态，确定新版本，更新 package.json 与文档、生成 CHANGELOG、提交 bump commit、创建 tag、推送 origin 与本地备份 remote，并在可用时确认 CI 是否触发。用户要求发版、升级版本号、更新更新日志、打 tag 或推送发布时使用。
---

# CC Desktop 版本发布

用于本仓库的正式发版流程。目标是稳定完成版本号更新、更新日志生成、Git 提交、打 tag、按 remote 规则推送 origin 与本地备份仓库，并把关键确认点交给用户，不静默跳过。

## 适用场景

- 用户要求“发版”“升级版本号”“出一个新版本”“打 tag 并推送”
- 用户要求同步更新 `package.json`、`CLAUDE.md`、`docs/CHANGELOG.md` 和文档头部版本号
- 用户要求按本仓库约定推送 `master`、发布 tag 与本地备份仓库
...
- 发版关键步骤必须严格串行执行，绝对不要把 `git commit`、`git tag`、`git push`、`gh run list` 放进并行工具调用里。
- 顺序必须固定为：先 `git commit` 成功，再 `git tag` 成功，再 `git push origin master` 成功，再 `git push origin vX.Y.Z` 成功，之后才允许推送本地备份 remote 或检查 CI。
- 每一步执行后都要重新读取并确认当前 `HEAD` / tag / remote 状态，再进入下一步；不要基于“上一步应该成功”的假设继续。
...
- 不要顺手提交无关改动。
- 提交完成后，必须单独执行 `git rev-parse --short HEAD` 或 `git log --oneline -1`，确认 bump commit 已成为当前 `HEAD`。
...
- `git tag` 必须在 bump commit 成功后单独执行，不能与 `git commit` 放在同一个并行批次里。
- 创建 tag 后，必须单独执行 `git show --no-patch --oneline vX.Y.Z`，确认 tag 指向当前 bump commit。

### 7. 推送到 origin 和本地备份 remote
...
- 推送顺序必须串行，且必须先完成 `origin/master`，再完成 `origin` 的单个版本 tag，最后才处理 `local` 等其他 remote。
- `git push origin master` 与 `git push origin vX.Y.Z` 之间必须插入状态确认：重新读取 `origin/master` 或至少确认本地 `HEAD` 未变化。
- 任何 remote 推送都不要和其他 remote 的推送并行执行。
