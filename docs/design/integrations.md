...
| 级别 | 存储位置 | 作用域 | 可编辑 |
|------|---------|--------|--------|
| **User** | `<Claude profile json>` → `mcpServers` | 跨项目共享 | 是 |
| **Local** | `<Claude profile json>` → `projects.<path>.mcpServers` | 项目私有 | 是 |
...
- User/Local → 读写 `<Claude profile json>`（原子写入 `atomicWriteJson`）
...
```json
{
  "env": {
    ...
    "NODE_OPTIONS": "-r \"<Claude profile>/proxy-support/proxy-setup.cjs\""
  }
}
```
...
1. **文件级禁用**（`toggleMcpDisabled`）：写入 `<Claude profile json>` 的 `projects.<path>.disabledMcpServers` 数组
...
| 级别 | 路径 | 调用方式 | 可编辑 |
|------|------|---------|--------|
| 插件级 (只读) | `<Claude profile>/plugins/{plugin}/skills/` | `/plugin-name:skill-id` | 否 |
| 用户全局 | `<Claude profile>/skills/` | `/skill-id` | 是 |
...
| 级别 | 路径 | 触发方式 | 可编辑 |
|------|------|---------|--------|
| 插件级 (只读) | `<Claude profile>/plugins/{plugin}/agents/` | 自动（Claude 根据 description 选择） | 否 |
| 用户全局 | `<Claude profile>/agents/` | 自动 | 是 |
...
- **全局**：`<Claude profile>/settings.json` → `hooks` 字段
...
- `<Claude profile>/plugins/installed_plugins.json` — 插件安装注册表
- `<Claude profile>/plugins/known_marketplaces.json` — 已注册市场源
- `<Claude profile>/settings.json` → `enabledPlugins` / `extraKnownMarketplaces` — 插件启停状态与额外市场来源
...
## 组件市场 (Capability Manager)

> 核心文件：`src/main/managers/capability-manager.js`、`src/main/managers/skills/market.js`

### 组件市场入口

组件市场（`ComponentMarketModal`）统一为全局入口，通过左下角齿轮菜单 → 能力管理工作台进入，支持 skill / agent / prompt / mcp 四类组件的安装与管理。

### 清单来源

`{registryUrl}/index.json` — 远程拉取，支持主源 + 镜像 fallback，包含 skills / agents / prompts / mcps 四类。

### Skill 安装机制

优先用 `npx skills add` 整目录克隆，失败自动 fallback 到逐文件 HTTP 下载：

```
installMarketSkill()
├── deriveGitCloneUrl(registryUrl)  → 派生 git clone URL
├── 优先：npx skills add <gitUrl> --skill <id> -g -a claude-code -y
└── 失败 → fallback：HTTP 逐文件下载（依赖 index.json 的 files 字段）
```

- Gitee raw URL 自动派生为 Gitee git clone 地址
- GitHub raw URL 自动派生为 GitHub git clone 地址
- `.market-meta.json` 两条路径均写入，保持版本检测正常工作
...
|------|-----------|----------|
| skill | `<Claude profile>/skills/{id}/SKILL.md` 存在 | `SKILL.md.disabled` 存在 |
| agent | `<Claude profile>/agents/{id}.md` 存在 | `{id}.md.disabled` 存在 |
...
| 级别 | 路径 | 作用域 |
|------|------|--------|
| Global | `<Claude profile>/settings.json` | 全局 |
