...
此前版本升级中曾出现个别旧会话无法按原方式恢复的情况。该问题不是本次项目/CWD 统一升级已确认造成的回归；本次升级涉及同一持久化链路，因此必须持续作为发布风险跟踪，不能仅以当前结构迁移测试通过而关闭。

当前实现会保留 `agent_conversations` 记录和未绑定会话的 `cwd` 回退，并对表重建使用局部事务；但尚未提供迁移前的 SQLite 一致性备份，也没有覆盖真实历史用户数据库、升级中断和旧 SDK 会话恢复的完整端到端验证。

关闭条件：

1. 首次破坏性迁移前自动创建可验证的 SQLite 一致性备份，并提供明确的恢复路径。
2. 使用脱敏的真实旧版本数据库副本，验证升级、重启、左栏发现、消息读取、IM 历史恢复和普通会话继续对话。
3. 覆盖迁移失败或中断后的重试/回滚场景，并验证 `integrity_check`、会话数和消息数。
4. 对已失效的 SDK resume 会话给出可见诊断，区分“历史消息可访问”和“模型上下文可续接”。

在上述条件满足前，不将该风险标记为已解决；遇到用户升级后无法恢复旧会话时，优先保留原始 `sessions.db` 和主进程日志，禁止通过删除数据库来恢复启动。
