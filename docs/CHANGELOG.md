...
### 新增 (Feat)
- 定时任务 MCP schema 与后端服务语义对齐，统一对外暴露 `firstRunAt` 与 `maxRuns`
- 为 Naive UI 接入应用级语言映射，时间选择器内置按钮随界面语言切换

### 修复 (Fix)
- 修复定时任务相关时间选择器在右侧面板窄布局下的弹层溢出
- 压缩定时任务详情页执行配置与调度设置区域的留白，减少无意义滚动
- 调整运行历史折叠态展示，移除冗余“展开”占位文案
