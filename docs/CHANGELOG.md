...
### 测试 (Test)
- **macOS 符号链接目录恢复回归**：测试显式模拟 Darwin 路径语义，避免 Linux CI 对 macOS 专属物理路径解析产生错误断言。
...
### 新增 (Feat)
- **工程目录绑定恢复**：当已绑定工程目录失效时，可选择新目录并在冲突检查、活动会话保护与失败回滚后，恢复工程身份、项目会话、定时任务和 Claude 历史数据的目录绑定。
- **默认工作台菜单收敛**：默认隐藏水文工作台演示和 Notebook 入口，仍可在全局设置中按需恢复显示，减少主界面无关入口。
...
- **内置 MCP 管理能力**：内置 Agent 能够读取、添加、更新和删除 Hydro Desktop 的 MCP 配置，并支持管理全局 MCP 工具权限，安装或新增 MCP 后可默认授权全部工具。
...
### 修复 (Fix)
- **MCP 权限体验收口**：MCP 权限开关恢复为单一状态按钮，权限变更后自动刷新列表；面向 Agent 暴露的 MCP 配置结果会脱敏环境变量与敏感字段。
- **桌面启动稳定性**：修复开发启动时偶发主窗口空白的问题，确保页面引导流程在异常条件下仍可恢复。
...
### 测试 (Test)
- **工作台回归断言同步**：更新嵌入工作台工具禁用规则与 IM 专用宿主路由的测试断言，匹配当前实现，恢复完整测试套件与发布 CI。
...
### 新增 (Feat)
- **项目树驱动的 Agent 会话组织**：左侧会话按 `projects` 项目树展示，会话通过项目身份归属；`projects.path` 成为目录身份来源，`cwd` 仅保留为运行时快照与旧数据兼容字段。
- **Notebook 独立工作台**：Notebook 从主窗口模式迁为独立、单例的工作台窗口；入口归入“内嵌应用”菜单，但仍保留 Notebook 专属的数据、会话与目录管理边界。

### 修复 (Fix)
- **历史项目与会话迁移**：补齐项目隐藏状态恢复、历史会话归属与 IM 分组兼容，避免旧会话因 `cwd` 或来源字段导致错误归类或丢失可见性。
- **跨平台路径身份**：修复 macOS 路径规范化与项目身份校验，保持项目路径在恢复、迁移和能力查询中的一致性。
- **Notebook IM 恢复**：同一 Notebook 会话同时收到创建与消息事件时只恢复一次；创建后若 Notebook 加载失败，不再显示成功提示。

### 文档 (Docs)
- **项目与工作台说明**：同步项目/CWD 升级设计、用户验收用例、Notebook 工作台协议、代码索引和当前开发进度文档。

### 测试 (Test)
- **项目身份与工作台回归**：补充项目树、历史迁移、IM 会话分组、Notebook 独立窗口、同会话恢复去重和加载失败提示的自动化覆盖。

### 其他 (Chore)
- **GitNexus 索引维护**：刷新代码图谱、关系统计与协作说明。
...
### 新增 (Feat)
- **Hydro Desktop 启动闪屏**：启动期间显示与现有应用图标一致的 Hydro Desktop 闪屏，在主进程服务和渲染器就绪后自动关闭，并提供超时兜底。

### 修复 (Fix)
- **内置定时能力兼容**：移除过时的 Claude Cron 工具禁用规则，避免内置定时能力被错误拦截。

### 其他 (Chore)
- **工具栏与运行事件整理**：优化工具栏图标显示，并隐藏不面向用户的 SDK 任务事件。
- **图标资源重构**：将应用图标整理为可复用的 SVG 资源。
- **GitNexus 索引维护**：刷新索引元数据与统计信息。
...
### 新增 (Feat)
- **内置 API 服务商预设**：首次安装自动创建千问 Token Plan 与 DeepSeek 配置，新增 API Profile 默认采用 `auth_token`，新用户仅需填写密钥即可使用。
- **会话应用筛选同步**：删除能力管理中的会话应用模板后，Agent 左侧筛选列表会即时刷新。

### 其他 (Chore)
- **旧配置与 IPC 清理**：移除失效模型映射和 Notebook 遗留 IPC，并修正主进程代码索引说明。
- **GitNexus 索引维护**：更新索引统计信息。
...
### 修复 (Fix)
- **历史数据库迁移**：兼容旧 Claude 历史扫描留下的 sync 项目绑定，避免升级后主进程数据库迁移被外键约束中断，修复启动空白页与 `embedded-app:list` IPC 未注册问题
...
### 新增 (Feat)
- **内置 Claude Agent SDK 升级**：将 bundled SDK 升级到 `0.3.206`，并补齐运行期依赖，确保打包给用户的 Claude runtime 同步升级
- **项目身份模型**：为 Agent 对话引入项目身份绑定，相关消费者可直接读取项目上下文与能力配置
- **项目化会话组织**：按项目组织 Agent 会话树，支持项目维度的会话分组与默认标题保留
- **内置 Claude runtime 绑定**：Agent 运行链路固定使用 bundled Claude，减少外部环境差异
- **设置工作台迁移**：将核心设置、工作台设置与更新入口迁移到主工作区，减少独立管理窗口跳转

### 修复 (Fix)
- **链接预览渲染**：补充 `nextTick` 导入，修复链接预览相关渲染时序问题
- **跨平台项目路径规范化**：在 Linux CI 等非 Windows 环境中正确识别 `C:/...` Windows 绝对路径，避免项目身份迁移测试误走 POSIX 校验
- **设置页交互**：恢复全局头部视觉层级，修复设置菜单多语言显示与重复打开管理窗口的问题
- **文件树状态保持**：删除文件后保留文件树展开状态
- **默认会话标题**：避免默认 Agent 会话标题被意外覆盖
- **Claude 连接提示**：隐藏 SDK thinking 遥测并抑制无害的 Claude connector 鉴权提示
- **项目列表清理**：移除开发者会话 join 对项目列表的影响

### 文档 (Docs)
- **Agent 项目化设计**：补充项目身份、项目基础设施和主工作区设置迁移设计说明
- **开发者模式退役说明**：同步 Agent 工作台叙事、项目隐藏可见性语义与清理阶段状态
- **GitNexus 索引说明**：同步索引元数据、符号统计与文档版本信息

### 测试 (Test)
- **开发者模式退役回归**：补强开发者模式退役相关 review 修复的测试覆盖

### 其他 (Chore)
- **开发者模式退役**：移除开发者模式 UI、数据链和 Claude 历史扫描链路，并对齐 Agent 项目基础设施
- **设置页布局整理**：统一全局设置操作区样式
- **GitNexus 索引维护**：多次刷新 GitNexus 索引统计和元数据
...
### 修复 (Fix)
- **IM 绑定会话 Slash 命令**：修复桌面端 IM 绑定会话无法使用 `/` 命令的问题，保持桌面输入框在底层 Agent 会话激活后可用

### 其他 (Chore)
- **GitNexus 索引统计**：同步更新 GitNexus 索引统计信息
...
### 新增 (Feat)
- 会话应用管理与运行时接入成型，支持创建、管理和启动专用会话应用。
- 会话应用接入 hydrodesktop 内置 MCP，支持在桌面内复用会话能力。

### 修复 (Fix)
- 会话应用启动时显式创建默认工作目录，避免 cwd 缺失导致的启动异常。
- 优化会话应用会话入口与历史会话打开路径，减少失效跳转。

### 文档 (Docs)
- 对齐会话应用用户指南与 macOS 兼容性检查文档。

### 其他 (Chore)
- 移除捆绑安装器中的 Claude 检测逻辑，简化安装链路。
...
### 新增 (Feat)
- 桥接 Agent 权限与问询交互到飞书、钉钉、企业微信，支持在 IM 中完成审批和选择。
- 建立统一 IM 附件预览底座，保留图片自动理解并接入文档附件预览。
- 支持飞书入站文件和文档附件保存、预览与本地路径传递。
- 支持飞书、钉钉、企业微信文档附件出站发送，覆盖 PDF、Word、Excel、PowerPoint。
- 内置 MCP `im_send` 支持通过 `filePaths` 从会话内向 IM 目标发送本地文档。

### 修复 (Fix)
- 加固 macOS / POSIX 本地文件预览路径，避免特殊字符导致 PDF / HTML webview 解析异常。

### 文档 (Docs)
- 同步 IM 附件传输设计、内置 MCP 能力说明和三端回归测试指南。

## v1.7.82 - 2026-06-15

### 修复 (Fix)
- 优化 Provider 关联 API 配置的交互体验

### 文档 (Docs)
- 修正 CHANGELOG 中 IM 桥接数量（5→3）

## v1.7.81 - 2026-06-14

### 新增 (Feat)
- 新增 session_get_current / session_match_task MCP 会话上下文工具，支持只读查询与定时任务会话匹配。
- 全量三端 IM 桥接（钉钉/企业微信/飞书）统一支持 base64 图片发送，微信通知通道同步重构。
- 优化 IM 快速发送工具栏交互流。

## v1.7.80 - 2026-06-12

### 新增 (Feat)
- 统一定时任务会话与普通会话的内置 IM MCP 注入逻辑，并补齐 IM 解绑能力。

### 修复 (Fix)
- 加固内置 IM 图片发送链路，统一本地图片校验与跨通道发送行为。

### 文档 (Docs)
- 清理定时任务会话相关文档表述，并更新飞书权限配置指南文案与示意图。

## v1.7.79 - 2026-06-11

### 新增 (Feat)
- 能力查询注入全量 IM 桥接实例（微信/钉钉/飞书/企业微信桥 + wecom-cli），支持按通道动态构建 MCP 配置。
- IM 桌面端干预文案支持可配置化，不再硬编码。

### 修复 (Fix)
- 加固内置 IM MCP 目标绑定行为，收紧通道匹配与目标校验逻辑，降低误绑定风险。

### 其他 (Chore)
- 渠道配置指南页面移除微信导航入口，仅保留钉钉/飞书/企业微信三端链接。
- 临时隐藏个人微信通道入口。

## v1.7.78 - 2026-06-11

### 修复 (Fix)
- 调整应用内 IM 配置指南打开方式，统一改为 Electron 原生 `shell.openPath`，提升 macOS 下本地帮助文档的打开兼容性与错误反馈稳定性。

### 文档 (Docs)
- 刷新钉钉、飞书、企业微信、微信的内置配置指南，补充申请入口、分步说明与截图资源，并确保打包后可随应用一起分发显示。

## v1.7.77 - 2026-06-10

### 修复 (Fix)
- 修复 `wecom-cli` 在打包应用中的执行环境，统一复用增强后的 PATH，避免 macOS 打包版将已安装 CLI 误判为未安装，并同步提升联系人检测链路稳定性。

## v1.7.76 - 2026-06-10

### 新增 (Feat)
- 统一四个 IM 通道的默认工作目录规则：默认继承全局 Agent 输出目录，并自动落到各自的独立子目录。
- 补全 `agent_conversations` 的 IM 身份迁移与初始化逻辑，统一收敛到 `im_user_id / im_chat_id / im_channel / im_chat_type` 字段模型。
- 新增微信桥运行态控制能力，便于从桌面端查看和调整微信桥接状态。

### 修复 (Fix)
- 统一钉钉、飞书、企业微信、微信的工作目录配置交互：显示绝对路径，支持浏览修改与恢复默认。
- 加固 IM 历史数据迁移过程，确保历史会话的 `cwd / cwd_auto` 保持不变，并兼容新旧字段混合数据。
- 窗口关闭时同步关闭内嵌应用会话，避免残留嵌入式会话占用。

### 重构 (Refactor)
- 抽取并统一 IM 绑定、身份、历史会话、标题与运行时注册 helper，收敛跨通道重复逻辑。
- 清理微信链路与诊断代码，统一单聊 key、历史菜单文案与图标资源组织方式。

## v1.7.74 - 2026-05-26

### 新增 (Feat)
- 飞书桥接完整实现：支持 Stream 模式接收消息、会话绑定、主动推送、交互式卡片、命令系统、图片收发。
- 钉钉主动推送直接发送：支持从桌面端主动向钉钉联系人发送文本消息。
- 统一 IM 主动发送绑定入口，钉钉/微信/飞书三端对齐。

### 修复 (Fix)
- 飞书会话绑定流程全面稳定化：精准去除机器人@、统一入站处理、显示名称解析、去重投递、消息解析、桌面端回复路由与标注。
- 飞书主动推送修复：目标枚举、恢复绑定、保持最新绑定会话、关闭卡片高亮清理。
- 飞书命令会话按键（chatId）正确重绑定。
- Agent 会话来源过滤器新增飞书支持。
- 钉钉主动回复路由恢复。
- 微信快速发送目标下拉列表锁定为已绑定目标，对齐钉钉/飞书行为。
- IM 会话绑定流程稳定化：标题修复、恢复绑定、注入保存的图片路径。
- API 配置表单选择 Provider 时自动选第一个模型。

### 重构 (Refactor)
- 统一 IM sendText + 会话绑定到 Bridge 层，消除 ChatInputToolbar 中的冗余预绑定调用。
...
### 修复 (Fix)
- 统一 Windows 所有图标源（托盘、任务栏、安装程序）使用同一套 ICO，各场景图标一致。
- 托盘图标 DPI 感知缩放（scaleFactor × 16），修复高 DPI 下托盘图标模糊问题。
- 安装程序添加 ManifestDPIAware 启用高 DPI 渲染，修复安装过程页面图标模糊。
- AI 生成锐利版应用图标，替换原有含辉光特效的模糊设计。
- 图标加入圆形透明裁切，修复快捷方式图标白底问题。
- 删除独立托盘图标文件（tray.png），统一由 icon.png 替代。
...
- 补充微信通知相关文案与测试约束，统一任务关联会话与普通会话的内置 MCP 注入语义
