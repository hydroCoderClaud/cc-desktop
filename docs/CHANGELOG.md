...
- 修复 Hooks 面板显示为空的问题
- 修复 Hook 保存失败 "An object could not be cloned" 错误
- 修复 AI 助手 API 配置列表添加后不刷新

### 🔧 改进

- Skills 标签页默认折叠分类
- 新增 `file:readJson` / `file:writeJson` IPC 处理器
...
- 展示插件组件（Commands/Agents/Skills/Hooks/MCP）
- 启用/禁用开关、卸载功能
- 搜索过滤、刷新、打开插件目录
...
- 添加 `busy_timeout = 5000` 解决数据库锁定问题
