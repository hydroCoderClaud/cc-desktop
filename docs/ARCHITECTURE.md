...
> v1.7.58+ | 本文档是架构设计的入口，概述整体设计和模块关系，详细实现见各子文档。
...
Hydro Desktop（仓库 / 包名仍为 `cc-desktop`）是独立的 Electron 桌面应用，围绕 Claude Code CLI 提供本地桌面工作流、Agent 对话、Notebook 工作台、桌面端定时任务、钉钉桥接和微信通知/桥接能力。
...
| IPC Handlers | 13+ | agent, plugin-domain, config, prompt, project, session, active-session, capability, notebook, update, dingtalk, scheduled-task/queue, weixin-notify |
| Managers | 32+ | capability, dingtalk-bridge, weixin-bridge, weixin-notify-service, scheduled-task-service, hooks, mcp, settings, agent-file, agent-query, notebook, skills/\*, agents/\* |
