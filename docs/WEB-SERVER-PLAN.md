# Hydro Desktop Web 服务端计划

> 远期/备用探索方案：本文创建于 2026-02-22，当前仍**未进入主线实现**。执行前需结合现有 Notebook、钉钉桥接和模式结构重新评审，不应直接按本文落地。

## 一、目标

将 Hydro Desktop 从纯桌面应用扩展为 **桌面 + Web 双端架构**，支持：
...
| Agent 会话配置 | 默认模型、system prompt、会话参数 | ~6 |
