...
function buildClaudeEnvVars(profile, configManager) {
  ...
  if (runtimeProfile.authToken && runtimeProfile.authToken.trim()) {
    ...
    if (authType === 'api_key') {
      ...
      envVars.ANTHROPIC_AUTH_TOKEN = runtimeProfile.authToken.trim()
  ...
  // API 基础 URL
  if (runtimeProfile.baseUrl && runtimeProfile.baseUrl.trim()) {
  ...
  // 超时设置
  if (runtimeProfile.requestTimeout && runtimeProfile.requestTimeout > 0) {
    envVars.API_TIMEOUT_MS = String(runtimeProfile.requestTimeout)
  }
  ...
  // 代理设置
  if (runtimeProfile.useProxy) {
    if (runtimeProfile.httpsProxy && runtimeProfile.httpsProxy.trim()) {
      envVars.HTTPS_PROXY = runtimeProfile.httpsProxy.trim()
      envVars.https_proxy = runtimeProfile.httpsProxy.trim()
    }
    if (runtimeProfile.httpProxy && runtimeProfile.httpProxy.trim()) {
      envVars.HTTP_PROXY = runtimeProfile.httpProxy.trim()
      envVars.http_proxy = runtimeProfile.httpProxy.trim()
    }
  }
