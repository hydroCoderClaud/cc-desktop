...
export function useAgentPanel({ projects: projectSource = null } = {}) {
  ...
  const isExternalImConversation = (conv) => {
    const isHiddenWorkspaceConversation = conv?.projectKind === 'workspace' &&
      !isVisibleProjectConversation(conv)
    return isAutoGeneratedImConversation(conv) &&
      !isVisibleProjectConversation(conv) &&
      !isHiddenWorkspaceConversation
  }
  ...
  const buildProjectConversationGroups = (sourceConversations) => {
    ...
    for (const conv of sourceConversations) {
      const projectKey = getConversationDirectoryKey(conv)
      // A hidden workspace remains in the database so reopening its path can
      // restore it, but neither it nor its bound history belongs in this tree.
      if (projectKey && conv?.projectKind === 'workspace' && !groupsByKey.has(projectKey)) {
        continue
      }
    ...
    // Legacy and unbound sessions remain discoverable without becoming
    // synthetic cwd roots or taking precedence over normal projects.
