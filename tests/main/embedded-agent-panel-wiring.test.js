...
describe('embedded agent panel wiring', () => {
  it('reuses the existing AgentChatTab instead of custom chat rendering', () => {
    ...
    expect(source).toContain(':session-title="currentSessionTitle"')
    expect(source).toContain(':session-source="currentSessionSource"')
    expect(source).toContain(':session-im-channel="currentSessionImChannel"')
    ...
    expect(source).toContain('currentSessionImChannel.value = typeof session?.imChannel === \'string\' ? session.imChannel : null')
    expect(source).toContain("currentSessionImChannel.value = latestSession?.imChannel !== undefined")
    expect(source).toContain("? (typeof latestSession.imChannel === 'string' ? latestSession.imChannel : null)")
    expect(source).toContain("const handleImBindingChanged = async (event) => {")
    expect(source).toContain("window.addEventListener('agent-session:im-binding-updated', handleImBindingChanged)")
    expect(source).toContain("window.removeEventListener('agent-session:im-binding-updated', handleImBindingChanged)")
  ...
  it('lets embedded hosts send current business context through AgentChatTab', () => {
    ...
    expect(composableSource).toContain('const liveSession = agentApi?.getAgentSession')
    expect(composableSource).toContain("if (liveSession?.status === 'streaming') {")
