...
describe('hydrology backend wiring', () => {
  it('exposes hydrology station APIs in preload', () => {
    ...
    expect(source).toContain("createHydrologyRealtimeObservation: (payload) => ipcRenderer.invoke('hydrology:realtime:createObservation', payload)")
    expect(source).toContain("updateHydrologyRealtimeObservation: (payload) => ipcRenderer.invoke('hydrology:realtime:updateObservation', payload)")
    expect(source).toContain("deleteHydrologyRealtimeObservation: (observationId) => ipcRenderer.invoke('hydrology:realtime:deleteObservation', observationId)")
    expect(source).toContain("deleteHydrologyRealtimeSlotObservations: (payload) => ipcRenderer.invoke('hydrology:realtime:deleteSlotObservations', payload)")
    expect(source).toContain("listHydrologyReviewTasks: (filters) => ipcRenderer.invoke('hydrology:review:listTasks', filters)")
    expect(source).toContain("deleteHydrologyReviewTask: (taskId) => ipcRenderer.invoke('hydrology:review:deleteTask', taskId)")
    expect(source).toContain("deleteHydrologyReviewTasks: (taskIds) => ipcRenderer.invoke('hydrology:review:deleteTasks', taskIds)")
    expect(source).toContain("runHydrologyQualityCheck: (payload) => ipcRenderer.invoke('hydrology:review:runQualityCheck', payload)")
    expect(source).toContain("getHydrologyLatestReviewRunSummary: (filters) => ipcRenderer.invoke('hydrology:review:getLatestRunSummary', filters)")
  ...
  it('registers hydrology station ipc routes in dedicated handlers', () => {
    ...
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:realtime:createObservation'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:realtime:updateObservation'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:realtime:deleteObservation'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:realtime:deleteSlotObservations'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:review:listTasks'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:review:deleteTask'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:review:deleteTasks'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:review:runQualityCheck'")
    expect(source).toContain("createIPCHandler(ipcMain, 'hydrology:review:getLatestRunSummary'")
  ...
  it('boots hydrology database and service from main ipc setup', () => {
    ...
    expect(source).toContain("new QualityCheckService({ stationService, realtimeService, hydrologyDatabase })")
  ...
  it('persists corrected slot value separately from manual slot value', () => {
    ...
    expect(source).toContain('CREATE TABLE IF NOT EXISTS hydrology_quality_check_runs')
    expect(source).toContain("scope_type TEXT NOT NULL DEFAULT 'station'")
    expect(source).toContain("ALTER TABLE hydrology_quality_check_runs ADD COLUMN scope_type TEXT NOT NULL DEFAULT 'station'")
    expect(source).toContain('checked_slot_count INTEGER NOT NULL DEFAULT 0')
    expect(source).toContain('saveQualityCheckRun(summary = {})')
    expect(source).toContain("getLatestQualityCheckRun(stationId, observationType, scopeType = 'station')")
